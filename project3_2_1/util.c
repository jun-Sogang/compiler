...
// insert
	...
	TreeNode *local_declarations = statement->child[0];
	TreeNode *statement_list = statement->child[1];
	TreeNode * parameter_list = parameter;
	while (local_declarations != NULL) {
	//	printf("%s\n", local_declarations->child[1]->attr.name);
	//  	printf("%d\n", local_declarations->child[1]->lineno);
		
		st_insert(local_declarations->child[1]->attr.name, 0, local_declarations->child[1]->lineno);
		local_declarations = local_declarations->sibling;
	}

	while(parameter_list != NULL){ 
		if (parameter_list->kind.decl != ParamVoidK)	
			st_insert(parameter_list->child[1]->attr.name, 0, parameter_list->child[1]->lineno);
    ...
    if (tree->nodekind == StmtK) {
      ...
      switch (tree->kind.exp) {
		...
		case CallK:
          ...
          fprintf(listing, "Call Procedure: %d\n", tree->child[5]->type);
